...
const updateProduct = asyncHandler(async (req, res) => {
  const { name, image, category, description, price, countInStock, orderQuantity } = req.body;

  // Find the product by ID
  const product = await Product.findById(req.params.id);

  if (product) {
    // Ensure order quantity is valid (positive number and does not exceed available stock)
    if (orderQuantity <= 0) {
      res.status(400);
      throw new Error('Order quantity must be greater than 0');
    }

    if (product.countInStock < orderQuantity) {
      res.status(400);
      throw new Error('Not enough stock available');
    }

    // Update product's stock count after an order
    product.countInStock -= orderQuantity;
    ...
    res.status(200).json({
      ...
      description: updatedProduct.description,
      price: updatedProduct.price,
      countInStock: updatedProduct.countInStock, // Include the updated stock count in the response
    });
  } else {
    res.status(404);
    throw new Error('Product not found');
